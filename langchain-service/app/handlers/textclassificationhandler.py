...
class TextClassificationHandler(BaseNodeHandler):
    def __init__(self, redis_service):
        ...
        logger.info("Initializing modular Text Classification Handler")
    ...
    async def execute(self, message: NodeExecutionMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            categories = node_data.get("categories", "positive, neutral, negative")

            execution_id = str(message.executionId)
            client = await self.llm_factory.get_llm_client(execution_id, 0.0, 100)

            prompt = f"Classify the following text into one of these categories: {categories}.\nReturn ONLY a JSON object containing a single key 'classification' with the chosen category.\n\nText:\n{text}"
            ...
            try:
                ...
                classification = res.strip()

            output = {**context, "original_text": text, "classification": classification, "node_type": "text-classification", "node_executed_at": datetime.now().isoformat()}
