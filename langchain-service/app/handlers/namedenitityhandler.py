...
class NamedEntityHandler(BaseNodeHandler):
    def __init__(self, redis_service):
        ...
        logger.info("Initializing modular Named Entity Handler")
    ...
    async def execute(self, message: NodeExecutionMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            entity_types = node_data.get("entity_types", "person, organization, location")

            execution_id = str(message.executionId)
            client = await self.llm_factory.get_llm_client(execution_id, 0.0, 500)

            prompt = f"Extract the following entities: {entity_types} from the text below.\nReturn ONLY a JSON object with keys as entity types and values as arrays of extracted strings.\n\nText:\n{text}"
            ...
            try:
                ...
                entities = json.loads(res.strip())
            except:
                pass

            output = {**context, "original_text": text, "entities": entities, "node_type": "named-entity", "node_executed_at": datetime.now().isoformat()}
