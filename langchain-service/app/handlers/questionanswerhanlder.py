...
class QuestionAnswerHandler(BaseNodeHandler):
    def __init__(self, redis_service):
        ...
        logger.info("Initializing modular Question Answer Handler")
    ...
    async def execute(self, message: NodeExecutionMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            if not question.strip() and "question" in context: question = context["question"]
            if not question.strip(): raise ValueError("No question provided")

            context_text = node_data.get("context_text", "")
            if not context_text and "context_text" in context: context_text = context["context_text"]
            ...
            answer = await loop.run_in_executor(None, _call_llm)

            output = {**context, "question": question, "answer": answer, "node_type": "question-answer", "node_executed_at": datetime.now().isoformat()}
