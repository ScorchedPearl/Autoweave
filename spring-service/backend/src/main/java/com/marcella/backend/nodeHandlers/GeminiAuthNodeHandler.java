...
@Service
@Slf4j
public class GeminiAuthNodeHandler implements NodeHandler {

    private final WorkflowEventProducer eventProducer;
    private final RedisTemplate<String, String> redisTemplate;
    private static final Duration API_KEY_TTL = Duration.ofHours(1);

    public GeminiAuthNodeHandler(WorkflowEventProducer eventProducer, 
    ...
    public boolean canHandle(String nodeType) {
        return "gemini-auth".equals(nodeType);
    }
    ...
    public Map<String, Object> execute(NodeExecutionMessage message) {
        ...
        log.info("Executing Gemini Auth node: {} for execution: {}", message.getNodeId(), executionId);
        ...
        try {
            ...
            if (apiKey != null && !apiKey.trim().isEmpty()) {
                String redisKey = "execution:" + executionId + ":gemini_api_key";
                redisTemplate.opsForValue().set(redisKey, apiKey, API_KEY_TTL);
                redisTemplate.opsForValue().set("gemini_api_key", apiKey, API_KEY_TTL);
                log.info("✅ Stored Gemini API key in Redis for execution: {}", executionId);
                output.put("gemini_auth", "success");
            } else {
                log.warn("⚠️ Gemini auth node missing 'api_key' in nodeData");
                output.put("gemini_auth", "failed - missing key");
            }
            ...
            log.error("❌ Gemini Auth node failed: {}", message.getNodeId(), e);
